...
def calculate_performance_metrics(data):
    ...
    if len(data) > 1:
        ...
        if "position" in data.columns:
            ...
            if trade_returns:
                metrics["win_rate"] = sum(1 for r in trade_returns if r > 0) / len(
                    trade_returns
                )
                metrics["avg_win"] = (
                    sum(r for r in trade_returns if r > 0)
                    / sum(1 for r in trade_returns if r > 0)
                    if sum(1 for r in trade_returns if r > 0) > 0
                    else 0
                )
                metrics["avg_loss"] = (
                    sum(r for r in trade_returns if r <= 0)
                ...
                metrics["profit_factor"] = (
                    ...
                    if sum(r for r in trade_returns if r <= 0) < 0
                    else float("inf")
                )
...
def grid_search(data, param_grid, n_jobs=-1):
    ...
    logger.info(
        f"Running grid search with {len(param_combinations)} parameter combinations"
    )
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            if (i + 1) % 10 == 0 or (i + 1) == len(param_combinations):
                logger.info(
                    f"Completed {i + 1}/{len(param_combinations)} parameter combinations"
                )
...
def visualize_optimization_results(results, output_dir=None):
    ...
    if "sharpe_ratio" in results_df.columns:
        ...
        if len(top_params) >= 2:
            plt.scatter(
                results_df[top_params[0]],
                results_df[top_params[1]],
