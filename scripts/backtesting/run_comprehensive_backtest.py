...
def load_sample_data(symbol="BTCUSDT", timeframe="1h"):
    ...
    try:
        ...
        if "timestamp" in data.columns:
            if (
                data["timestamp"].dtype == np.int64
                or data["timestamp"].dtype == np.float64
            ):
...
def calculate_performance_metrics(data):
    ...
    for i in range(1, len(data)):
        ...
        if position == 0:
            ...
            data.loc[data.index[i], "portfolio_value"] = data["portfolio_value"].iloc[
                i - 1
            ]
        else:
    ...
    if len(data) > 1:
        ...
        if not trades_df.empty:
            ...
            metrics["avg_win"] = (
                trades_df.loc[trades_df["return"] > 0, "return"].mean()
...
def visualize_backtest_results(
    data, trades_df, metrics, symbol, timeframe, output_dir=None
):
    """
    ...
    if not trades_df.empty:
        ...
        plt.bar(
            range(len(trades_df)),
            trades_df["return"],
            color=["green" if r > 0 else "red" for r in trades_df["return"]],
        )
...
def main():
    ...
    for symbol in args.symbols:
        for timeframe in args.timeframes:
            ...
            metrics, data_with_metrics, trades_df = calculate_performance_metrics(
                backtest_result
            )

            # Visualize results
            visualize_backtest_results(
                data_with_metrics, trades_df, metrics, symbol, timeframe, results_dir
            )

            # Find image files
            image_files = list(
                results_dir.glob(f"{symbol}_{timeframe}_*_{timestamp}.png")
            )
            ...
            # Save backtest data
            backtest_data_file = (
                results_dir / f"{symbol}_{timeframe}_backtest_data_{timestamp}.csv"
            )
            ...
            if not trades_df.empty:
                trades_data_file = (
                    results_dir / f"{symbol}_{timeframe}_trades_{timestamp}.csv"
                )
