...
trading_data = {"BTCUSDT": pd.DataFrame(), "ETHUSDT": pd.DataFrame()}
trading_signals = {"BTCUSDT": pd.DataFrame(), "ETHUSDT": pd.DataFrame()}
trading_positions = {"BTCUSDT": 0, "ETHUSDT": 0}
                ...
                if (current_time - last_update_time).total_seconds() >= 60:
                    for pair in trading_pairs:
                        ...
                        if new_data is not None and not new_data.empty:
                            ...
                            if (
                                latest_signal == 1 and current_position <= 0
                            ):  # Buy signal
                                # Execute buy order
                                ...
                                logger.info(
                                    f"BUY signal for {pair} at {trade['price']}"
                                )
                                ...
                                logger.info(
                                    f"SELL signal for {pair} at {trade['price']}"
                                )
    ...
    def _fetch_market_data(self, symbol, timeframe):
        ...
        try:
            ...
            if self.mode == "real" and self.credentials:
                ...
                if data_file.exists():
                    data = pd.read_csv(
                        data_file, index_col="timestamp", parse_dates=True
                    )
                else:
...
def update_dashboard(n_intervals, selected_pair):
    ...
    if selected_pair in trading_data and not trading_data[selected_pair].empty:
        ...
        if (
            selected_pair in trading_signals
            and not trading_signals[selected_pair].empty
        ):
...
def update_strategy_parameters(
    n_clicks, fast_ma_type, slow_ma_type, fast_period, slow_period, use_filters
):
