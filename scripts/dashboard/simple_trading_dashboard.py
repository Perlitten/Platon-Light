        ...
        while self.running:
            try:
                ...
                if (current_time - last_update_time).total_seconds() >= 10:
                    ...
                    logger.info(f"Simulated {trade_type.upper()} signal for {pair} at {price:.2f}")
...
def update_dashboard(n_intervals, selected_pair):
    ...
    balance = trading_balance + np.random.normal(0, 50) if trading_active else trading_balance
    ...
    if trading_history:
        ...
        for trade in trading_history[-10:]:
            ...
            trade_type_class = "text-success" if trade['type'] == 'buy' else "text-danger"
            
            history_rows.append(html.Tr([
                html.Td(trade_time),
                html.Td(trade['pair']),
                html.Td(html.Span(trade['type'].upper(), className=trade_type_class)),
                html.Td(f"${trade['price']:.2f}"),
                html.Td(f"{trade['amount']:.4f}"),
                html.Td(trade['mode'])
            ]))
        
        history_table = html.Table(history_rows, className="table table-striped table-dark")
    else:
