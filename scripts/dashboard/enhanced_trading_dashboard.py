...
for pair in trading_pairs:
    ...
    sample_data[pair].loc[(sample_data[pair]['fast_ma'] > sample_data[pair]['slow_ma']) & 
                          (sample_data[pair]['fast_ma'].shift(1) <= sample_data[pair]['slow_ma'].shift(1)) &
...
class EnhancedTradingEngine:
    ...
    def start(self):
        ...
        logger.info(f"Trading engine started in {self.mode} mode with {self.risk_level} risk level")
                ...
                if (current_time - last_update_time).total_seconds() >= 10:
                    ...
                    for pair in trading_pairs:
                        ...
                        if pair in self.positions:
                            ...
                            elif (position_type == 'buy' and current_signal == -1) or (position_type == 'sell' and current_signal == 1):
                                should_close = True
                            ...
                            if should_close:
                                ...
                                trade = {
                                    ...
                                    'pair': pair,
                                    'type': 'sell' if position_type == 'buy' else 'buy',  # Closing action
                                    'price': float(price),
                                ...
                                logger.info(f"Closed {position_type.upper()} position for {pair} at {price:.2f} with PnL: {pnl:.2f} ({pnl_pct*100:.2f}%) - Reason: {close_reason}")
                        ...
                        # Check for new entry signals if we don't have a position
                            ...
                            position_size_pct = risk_levels[self.risk_level]['position_size']
                            ...
                            logger.info(f"Opened {position_type.upper()} position for {pair} at {price:.2f}")
    ...
    def _calculate_current_metrics(self):
        ...
        if self.performance['peak_balance'] > 0:
            current_drawdown = (self.performance['peak_balance'] - current_balance) / self.performance['peak_balance']
    ...
    def _update_performance_metrics(self):
        ...
        performance_metrics['max_drawdown'] = self.performance['max_drawdown'] * 100  # Convert to percentage
        
        # Calculate daily return (simplified)
        if trading_balance > 0:
            performance_metrics['daily_return'] = (trading_balance / 10000.0 - 1) * 100  # Assuming 10000 initial balance
            performance_metrics['monthly_return'] = performance_metrics['daily_return'] * 30  # Simplified
        
        # Calculate Sharpe ratio (simplified)
        if performance_metrics['daily_return'] > 0:
            performance_metrics['sharpe_ratio'] = performance_metrics['daily_return'] / (performance_metrics['max_drawdown'] + 0.01)  # Avoid division by zero
        else:
    ...
    [
        ...
        dbc.Row(
            [
                dbc.Col(
                    [
                        html.H1("Platon Light Trading Dashboard", className="text-center mb-4"),
                        html.H5(f"Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", id="last-update-time", className="text-center text-muted mb-4"),
                    ],
                    width=12,
                )
            ],
            ...
            [
                ...
                dbc.Col(
                    [
                        dbc.Card(
                            [
                                dbc.CardHeader(html.H4("Trading Controls", className="text-center")),
                                dbc.CardBody(
                                    [
                                        ...
                                        html.Div(
                                            [
                                                ...
                                                dcc.RadioItems(
                                                    ...
                                                    options=[
                                                        {"label": "Dry Run Mode", "value": "dry_run"},
                                                        {"label": "Real Trading Mode", "value": "real"},
                                                    ],
                                                    value="dry_run",
                                                    className="mb-3",
                ...
                dbc.Col(
                    [
                        dbc.Card(
                            [
                                dbc.CardHeader(html.H4("Account Overview", className="text-center")),
                                dbc.CardBody(
                                    [
                                        # Balance Display
                                        html.Div(
                ...
                dbc.Col(
                    [
                        dbc.Card(
                            [
                                dbc.CardHeader(html.H4("Risk Metrics", className="text-center")),
                                dbc.CardBody(
                                    [
                                        # Risk Metrics Table
                                        dash_table.DataTable(
                                            ...
                                            data=[
                                                ...
                                                {"metric": "Sharpe Ratio", "value": f"{performance_metrics['sharpe_ratio']:.2f}"},
                                                {"metric": "Daily Return", "value": f"{performance_metrics['daily_return']:.2f}%"},
                                                {"metric": "Monthly Return", "value": f"{performance_metrics['monthly_return']:.2f}%"},
                                            ],
                                            style_header={
                                            ...
                                            style_data_conditional=[
                                                {
                                                    "if": {"row_index": 0},  # Max Drawdown
                                                    "color": "red" if max_drawdown > 0.1 else "orange" if max_drawdown > 0.05 else "green",
                                                },
                                                {
            ...
            [
                ...
                dbc.Col(
                    [
                        dbc.Card(
                            [
                                dbc.CardHeader(html.H4("Price Chart", className="text-center")),
                                dbc.CardBody(
                                    [
                                        dcc.Graph(
                                            id="price-chart",
                ...
                dbc.Col(
                    [
                        dbc.Card(
                            [
                                dbc.CardHeader(html.H4("Trading History", className="text-center")),
                                dbc.CardBody(
                                    [
                                        dash_table.DataTable(
        ...
        for trade in trading_history:
            if trade['pair'] == selected_pair:
                ...
                if 'type' in trade and trade['type'] == 'buy':
                    ...
                    pnl_text = f" ({trade['pnl_pct']:.2f}%)" if 'pnl_pct' in trade else ""
...
def update_performance_chart(n_intervals):
    ...
    if trading_history:
        ...
        if timestamps:
            ...
            prev_sum = 0
            for i, trade in enumerate(sorted(trading_history, key=lambda x: x['timestamp'])):
                if 'pnl' in trade:
            ...
            balance_values = [10000 + pnl for pnl in cumulative_pnl]  # Assuming 10000 initial balance
...
def update_trading_history(n_intervals):
    ...
    for trade in trading_history:
        trade_data = {
            "time": datetime.fromisoformat(trade['timestamp']).strftime("%Y-%m-%d %H:%M:%S"),
            "pair": trade['pair'],
...
def update_account_overview(n_intervals):
    ...
    return balance_text, balance_class, status_text, status_class, str(positions_count), update_time
...
def toggle_trading(start_clicks, stop_clicks, mode, initial_balance, risk_level_value, telegram_enabled):
    ...
    if button_id == "start-trading-button" and not trading_active:
        ...
        if telegram_notifications_enabled and TelegramBot:
            try:
                ...
                threading.Thread(target=lambda: asyncio.run(telegram_bot.start()), daemon=True).start()
                
                # Send startup notification
                threading.Thread(target=lambda: asyncio.run(telegram_bot.send_message(
        ...
        if telegram_bot:
            try:
                ...
                # Stop bot
                threading.Thread(target=lambda: asyncio.run(telegram_bot.stop()), daemon=True).start()
            except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Platon Light Enhanced Trading Dashboard")
    parser.add_argument("--mode", choices=["dry_run", "real"], default="dry_run", help="Trading mode")
    ...
    print(f"Starting Platon Light Enhanced Trading Dashboard in {trading_mode.upper()} mode")
