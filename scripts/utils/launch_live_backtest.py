...
class SecureCredentialManager:
    ...
    def __init__(self, key_file=None, env_file=None):
        ...
        self.env_file = env_file or Path.home() / '.platon' / 'credentials.env.encrypted'
    ...
    def setup_credentials_interactive(self):
        ...
        use_telegram = input("Do you want to set up Telegram notifications? (y/n): ").strip().lower() == 'y'
        if use_telegram:
...
def fetch_market_data(symbol, timeframe, start_date, end_date, api_key=None, api_secret=None):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Launch live backtests with real market data')
    ...
    parser.add_argument('--setup-credentials', action='store_true', help='Set up API credentials')
    ...
    if not credentials:
        logger.warning("No credentials found. Run with --setup-credentials to set up API credentials.")
    ...
    output_file = output_dir / f"live_backtest_{args.symbol}_{args.timeframe}_{timestamp}.csv"
    ...
    from analyze_backtest_performance import calculate_performance_metrics, visualize_performance, generate_performance_report
