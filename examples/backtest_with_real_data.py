...
def parse_arguments():
    ...
    parser.add_argument('--fast-ma-type', default='EMA', choices=['SMA', 'EMA'], help='Fast MA type')
    parser.add_argument('--slow-ma-type', default='SMA', choices=['SMA', 'EMA'], help='Slow MA type')
    parser.add_argument('--use-filters', action='store_true', help='Use RSI and Bollinger Bands filters')
...
def calculate_performance_metrics(result_df):
    ...
    df['drawdown'] = df['strategy_cumulative_returns_peak'] - df['strategy_cumulative_returns']
    ...
    avg_win = df[df['trade_returns'] > 0]['trade_returns'].mean() if winning_trades > 0 else 0
    avg_loss = df[df['trade_returns'] < 0]['trade_returns'].mean() if losing_trades > 0 else 0
    
    profit_factor = abs(df[df['trade_returns'] > 0]['trade_returns'].sum() / 
                        df[df['trade_returns'] < 0]['trade_returns'].sum()) if losing_trades > 0 else float('inf')
    ...
    sharpe_ratio = (df['strategy_returns'].mean() / df['strategy_returns'].std() * 
                    (252 ** 0.5)) if df['strategy_returns'].std() > 0 else 0
...
def plot_results(df, metrics, symbol, timeframe):
    ...
    plt.plot(df.index, df['fast_ma'], label=f"{df['fast_ma_type'].iloc[0]} {df['fast_period'].iloc[0]}")
    plt.plot(df.index, df['slow_ma'], label=f"{df['slow_ma_type'].iloc[0]} {df['slow_period'].iloc[0]}")
    ...
    # Add drawdown
    plt.fill_between(df.index, 0, -df['drawdown_pct'], color='red', alpha=0.3, label='Drawdown')
...
def main():
    ...
    logger.info(f"Starting backtest for {args.symbol} {args.timeframe} from {args.start_date} to {args.end_date}")
    ...
    if not credentials:
        logger.warning("No credentials found. Run 'python launch_live_backtest.py --setup-credentials' to set up API credentials.")
    ...
    if data is None or len(data) == 0:
        logger.error("Failed to fetch market data or no data available for the specified period")
        return
    ...
    if args.save_results:
        ...
        output_file = output_dir / f"backtest_{args.symbol}_{args.timeframe}_{timestamp}.csv"
    ...
    if args.plot:
        plt_obj = plot_results(result_with_metrics, metrics, args.symbol, args.timeframe)
        
        if args.save_results:
            plot_file = output_dir / f"backtest_{args.symbol}_{args.timeframe}_{timestamp}.png"
