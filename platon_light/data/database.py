...
class DatabaseManager:
    ...
    def store_error(
        self, source: str, error: str, details: Optional[Dict] = None
    ) -> int:
        """
    ...
    def get_daily_performance(self, days: int = 7) -> List[Dict]:
        ...
        try:
            ...
            now = datetime.now()
            start_date = datetime(now.year, now.month, now.day) - datetime.timedelta(
                days=days - 1
            )
            ...
            for row in cursor.fetchall():
                ...
                win_rate = (
                    (winning_trades / trade_count) * 100 if trade_count > 0 else 0
                )
    ...
    def export_trades(
        self, format: str = "csv", file_path: Optional[str] = None
    ) -> Optional[str]:
        """
