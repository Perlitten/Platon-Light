...
class MarketDataManager:
    ...
    def __init__(self, config: Dict, exchange: ExchangeClient):
        ...
        # Order book settings
        self.order_book_depth = config.get("advanced", {}).get("market_data", {}).get("order_book_depth", 20)
        ...
        for tf in self.timeframes:
            if not self._is_custom_timeframe(tf):
                ...
                if klines:
                    ...
                    self.logger.debug(f"Loaded {len(formatted_data)} {tf} candles for {symbol}")
                else:
            ...
            try:
                ...
                if current_time - last_updates["price"] >= update_intervals["price"]:
                    ...
                    if price:
                        ...
                        await self._update_custom_candles(symbol, price, int(current_time * 1000))
                        
                    last_updates["price"] = current_time
                    
                # Update order book
                if current_time - last_updates["order_book"] >= update_intervals["order_book"]:
                    order_book = await self.exchange.get_order_book(symbol, self.order_book_depth)
                    if order_book:
                        self.order_books[symbol] = order_book
                ...
                if current_time - last_updates["ohlcv"] >= update_intervals["ohlcv"]:
                    for tf in self.timeframes:
                        if not self._is_custom_timeframe(tf):
                            ...
                            # Load latest klines
                            klines = await self.exchange.get_klines(symbol, interval, limit=100)
    ...
    def get_latest_data(self, symbol: str) -> Dict:
        ...
        # Add latest candle
        for tf in self.timeframes:
            if symbol in self.ohlcv_data and tf in self.ohlcv_data[symbol] and self.ohlcv_data[symbol][tf]:
    ...
    def update_win_rate(self, symbol: str, win: bool):
        ...
        self.win_rates[symbol] = self.win_rates[symbol] * (1 - alpha) + (1 if win else 0) * alpha
