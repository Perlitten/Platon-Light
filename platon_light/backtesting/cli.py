...
def parse_args():
    ...
    parser.add_argument("--strategy-params", type=str, help="Strategy parameters (JSON string)")
    ...
    # Data parameters
    parser.add_argument("--download-data", action="store_true", help="Download historical data")
    ...
    parser.add_argument("--strategies", type=str, help="Comma-separated list of strategy names to compare")
...
def update_config_from_args(config: Dict, args) -> Dict:
    ...
    if args.initial_capital:
        updated_config.setdefault("backtesting", {})["initial_capital"] = args.initial_capital
...
def run_backtest(config: Dict, args) -> Dict:
    ...
    if args.download_data:
        print(f"Downloading historical data for {symbol} ({timeframe}) from {start_date} to {end_date}")
        data_loader.download_data(symbol, timeframe, start_date, end_date)
        
    # Run backtest
    print(f"Running backtest for {symbol} ({timeframe}) from {start_date} to {end_date}")
    ...
    if args.report:
        ...
        report_summary = backtest_engine.generate_report(analysis, output_dir=args.output_dir)
...
def print_summary(results: Dict):
    ...
    print(f"Net Profit: {metrics.get('net_profit', 0):.2f} ({metrics.get('return_percent', 0):.2f}%)")
