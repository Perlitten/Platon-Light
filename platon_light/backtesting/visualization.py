...
class BacktestVisualizer:
    ...
    def plot_equity_curve(self, results: Dict, save: bool = True) -> plt.Figure:
        ...
        # Plot equity curve
        ax.plot(df["datetime"], df["equity"], linewidth=2, color=self.colors[0], label="Equity")
        
        # Plot initial capital
        initial_capital = results.get("metrics", {}).get("initial_capital", 0)
        ax.axhline(y=initial_capital, linestyle="--", color="gray", alpha=0.7, label="Initial Capital")
    ...
    def plot_drawdown(self, results: Dict, save: bool = True) -> plt.Figure:
        ...
        # Plot drawdown
        ax.fill_between(df["datetime"], df["drawdown"], 0, color=self.colors[1], alpha=0.3)
    ...
    def plot_trade_distribution(self, results: Dict, save: bool = True) -> plt.Figure:
        ...
        # Plot histogram
        sns.histplot(df["profit_percent"], bins=50, kde=True, ax=ax, color=self.colors[2])
    ...
    def plot_monthly_returns(self, results: Dict, save: bool = True) -> plt.Figure:
        ...
        # Plot heatmap
        sns.heatmap(monthly_returns_matrix, annot=True, fmt=".2%", cmap="RdYlGn", center=0, ax=ax)
    ...
    def plot_performance_metrics(self, results: Dict, save: bool = True) -> plt.Figure:
        ...
        # Plot bar chart
        bars = ax.bar(filtered_metrics.keys(), filtered_metrics.values(), color=self.colors)
    ...
    def plot_trade_analysis(self, results: Dict, save: bool = True) -> plt.Figure:
        ...
        axes[0, 0].scatter(range(len(df)), df["profit_percent"], alpha=0.6, color=self.colors[0])
        ...
        axes[0, 1].plot(range(len(df)), df["cumulative_profit"], linewidth=2, color=self.colors[1])
        ...
        axes[1, 0].bar(profit_by_side.index, profit_by_side.values, color=self.colors[2:4])
        ...
        if "duration" in df.columns:
            sns.histplot(df["duration"] / 60, bins=30, kde=True, ax=axes[1, 1], color=self.colors[4])
            axes[1, 1].set_title("Trade Duration Distribution")
            axes[1, 1].set_xlabel("Duration (minutes)")
            axes[1, 1].set_ylabel("Frequency")
            axes[1, 1].grid(True, alpha=0.3)
        else:
            axes[1, 1].text(0.5, 0.5, "No duration data available", ha="center", va="center")
            axes[1, 1].set_title("Trade Duration Distribution")
    ...
    def plot_strategy_comparison(self, results_list: List[Dict], strategy_names: List[str], 
        ...
        for i, (results, name) in enumerate(zip(results_list, strategy_names)):
            ...
            axes[0, 0].plot(df["datetime"], df["normalized_equity"], linewidth=2, 
        ...
        for i, (results, name) in enumerate(zip(results_list, strategy_names)):
            ...
            axes[0, 1].plot(df["datetime"], df["drawdown"], linewidth=2, 
                          label=name, color=self.colors[i % len(self.colors)])
        ...
        if metrics_data:
            ...
            df_metrics.plot(kind="bar", ax=axes[1, 0], color=self.colors[:len(metrics_to_compare)])
            ...
            axes[1, 0].text(0.5, 0.5, "No metrics data available", ha="center", va="center")
            axes[1, 0].set_title("Performance Metrics Comparison")
        ...
        if trade_data:
            ...
            bars1 = ax1.bar(df_trades["Strategy"], df_trades["Total Trades"], 
                          color=self.colors[0], alpha=0.7, label="Total Trades")
            bars2 = ax2.bar(df_trades["Strategy"], df_trades["Win Rate (%)"], 
                          color=self.colors[1], alpha=0.7, label="Win Rate (%)", width=0.5)
            ...
            axes[1, 1].text(0.5, 0.5, "No trade data available", ha="center", va="center")
    ...
    def create_report(self, results: Dict, output_path: Optional[str] = None) -> str:
        ...
        for i, fig in enumerate([equity_fig, drawdown_fig, trade_dist_fig, 
                               monthly_returns_fig, metrics_fig, trade_analysis_fig]):
            if fig:
