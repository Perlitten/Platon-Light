...
class StrategyOptimizer:
    ...
    def __init__(self, config: Dict):
        ...
        output_dir = config.get("backtesting", {}).get("output_dir", "optimization_results")
        ...
        self.fitness_metric = self.optimization_params.get("fitness_metric", "sharpe_ratio")
        self.max_workers = self.optimization_params.get("max_workers", multiprocessing.cpu_count())
        
        self.logger.info("Strategy optimizer initialized")
        
    def grid_search(self, param_grid: Dict, symbol: str, timeframe: str, 
        ...
        if self.max_workers > 1:
            with multiprocessing.Pool(self.max_workers) as pool:
                ...
                for params in param_combinations:
                    ...
                    args_list.append((param_dict, symbol, timeframe, start_date, end_date))
            ...
            for params in param_combinations:
                param_dict = dict(zip(param_names, params))
                result = self._run_backtest_with_params(param_dict, symbol, timeframe, start_date, end_date)
    ...
    def genetic_algorithm(self, param_ranges: Dict, symbol: str, timeframe: str, 
                         start_date: datetime, end_date: datetime, 
                         population_size: int = 50, generations: int = 10,
                         mutation_rate: float = 0.1, crossover_rate: float = 0.7) -> Dict:
        """
        ...
        if self.max_workers > 1:
            with multiprocessing.Pool(self.max_workers) as pool:
                ...
                for individual in population:
                    args_list.append((individual, symbol, timeframe, start_date, end_date))
            ...
            # Run backtests sequentially
            for individual in population:
                fitness = self._evaluate_individual(individual, symbol, timeframe, start_date, end_date)
        ...
        best_result = self._run_backtest_with_params(best_individual, symbol, timeframe, start_date, end_date)
    ...
    def walk_forward_optimization(self, param_grid: Dict, symbol: str, timeframe: str, 
                                 start_date: datetime, end_date: datetime, 
                                 window_size: int = 30, step_size: int = 7) -> Dict:
        """
        ...
        for metric in ["return_percent", "sharpe_ratio", "max_drawdown_percent", "win_rate"]:
            in_sample_values = [r["in_sample"]["metrics"].get(metric, 0) for r in results]
            out_sample_values = [r["out_sample"]["metrics"].get(metric, 0) for r in results]
    ...
    def _run_backtest_with_params(self, params: Dict, symbol: str, timeframe: str, 
                                 start_date: datetime, end_date: datetime) -> Dict:
        """
    ...
    def _evaluate_individual(self, individual: Dict, symbol: str, timeframe: str, 
        ...
        try:
            # Run backtest
            result = self._run_backtest_with_params(individual, symbol, timeframe, start_date, end_date)
        ...
        for _ in range(population_size):
            ...
            for param_name, param_range in param_ranges.items():
                ...
                if isinstance(min_val, int) and isinstance(max_val, int) and isinstance(step, int):
                    # Integer parameter
                    individual[param_name] = random.randrange(min_val, max_val + 1, step)
                else:
                    # Float parameter
    ...
    def _mutate(self, individual: Dict, param_ranges: Dict, mutation_rate: float) -> Dict:
        """
        ...
        for param_name, param_value in individual.items():
            ...
            if random.random() < mutation_rate:
                ...
                if isinstance(min_val, int) and isinstance(max_val, int) and isinstance(step, int):
                    ...
                    # Float parameter
                    mutated[param_name] = min_val + random.random() * (max_val - min_val)
    ...
    def _save_walk_forward_results(self, results: List[Dict]):
        ...
        try:
            ...
            for result in results:
                ...
                for metric_name, metric_value in result["out_sample"]["metrics"].items():
                    row[f"out_sample_{metric_name}"] = metric_value
                    
                # Add best parameters
                for param_name, param_value in result["in_sample"]["best_params"].items():
    ...
    def optimize_multi_objective(self, param_grid: Dict, symbol: str, timeframe: str, 
                               start_date: datetime, end_date: datetime, 
    ...
    def _find_pareto_front(self, results: List[Dict], objectives: List[str]) -> List[Dict]:
        ...
        for i, values1 in enumerate(objective_values):
            ...
            for j, values2 in enumerate(objective_values):
                ...
                if all(v2 >= v1 for v1, v2 in zip(values1, values2)) and any(v2 > v1 for v1, v2 in zip(values1, values2)):
                    is_dominated = True
                    break
