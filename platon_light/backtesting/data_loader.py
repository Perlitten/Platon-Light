...
class DataLoader:
    ...
    def __init__(self, config: Dict):
        ...
        # Get data directory
        self.data_dir = Path(config.get("backtesting", {}).get("data_dir", "data/historical"))
        ...
        try:
            ...
            if use_api:
                ...
                self.exchange = ccxt.binance({
                    ...
                    'options': {
                        'defaultType': 'future' if self.config.get("trading", {}).get("mode") == "futures" else 'spot'
                    }
                })
                ...
                self.exchange = None
                self.logger.debug("Exchange API client not initialized (using local data only)")
        except Exception as e:
    ...
    def load_data(self, symbol: str, timeframe: str, start_date: datetime, 
                 end_date: datetime, use_cache: bool = True) -> pd.DataFrame:
        """
        ...
        # No data found
        self.logger.warning(f"No data found for {symbol} ({timeframe}) from {start_date} to {end_date}")
        return pd.DataFrame()
    ...
    def _save_to_cache(self, symbol: str, timeframe: str, 
                      start_ts: int, end_ts: int, data: pd.DataFrame):
        """
        Save data to cache
        ...
        try:
            ...
            ''', (
                symbol, timeframe, start_ts, end_ts, data_bytes, int(datetime.now().timestamp())
            ))
            
            conn.commit()
            conn.close()
            
            self.logger.debug(f"Saved data to cache: {symbol} {timeframe} {start_ts}-{end_ts}")
        except Exception as e:
    ...
    def download_data(self, symbol: str, timeframe: str, start_date: datetime, 
                     end_date: datetime, save_csv: bool = True) -> pd.DataFrame:
        """
        ...
        try:
            ...
            self.logger.info(f"Downloading data for {symbol} ({timeframe}) from {start_date} to {end_date}")
    ...
    def prepare_data(self, data: pd.DataFrame, add_indicators: bool = True) -> pd.DataFrame:
        """
