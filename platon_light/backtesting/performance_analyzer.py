...
class PerformanceAnalyzer:
    ...
    def _calculate_additional_metrics(self, trades_df: pd.DataFrame, 
        ...
        try:
            ...
            if not trades_df.empty:
                ...
                if 'pnl' in trades_df.columns:
                    ...
                    # Calculate streaks
                    trades_df['streak_group'] = (trades_df['is_win'] != trades_df['is_win'].shift()).cumsum()
                    streak_counts = trades_df.groupby(['streak_group', 'is_win']).size()
                    
                    # Get max consecutive wins/losses
                    max_wins = streak_counts[streak_counts.index.get_level_values('is_win')].max() if True in streak_counts.index.get_level_values('is_win') else 0
                    max_losses = streak_counts[~streak_counts.index.get_level_values('is_win')].max() if False in streak_counts.index.get_level_values('is_win') else 0
                ...
                if 'symbol' in trades_df.columns and 'pnl' in trades_df.columns:
                    ...
                    symbol_performance['win_rate'] = symbol_performance['win_rate'] * 100
                    
                    metrics['symbol_performance'] = symbol_performance.to_dict(orient='index')
            
            # Calculate equity curve metrics
            if not equity_df.empty:
                # Convert datetime if it's a string
                if 'datetime' in equity_df.columns and equity_df['datetime'].dtype == 'object':
                ...
                if 'datetime' in equity_df.columns and 'equity' in equity_df.columns:
                    ...
                    risk_free_rate = self.config.get("backtesting", {}).get("risk_free_rate", 0.0)
                    
                    avg_daily_return = daily_equity['return'].mean()
                    excess_return = avg_daily_return - (risk_free_rate / 252)
                    
                    if metrics['daily_volatility'] > 0:
                        metrics['sharpe_ratio'] = (excess_return / metrics['daily_volatility']) * np.sqrt(252)
                    else:
                        metrics['sharpe_ratio'] = 0
                        
                    # Sortino ratio (downside deviation)
                    negative_returns = daily_equity['return'][daily_equity['return'] < 0]
                    ...
                    if 'max_drawdown_percent' in metrics and metrics['max_drawdown_percent'] > 0:
                        annualized_return = avg_daily_return * 252
                        metrics['calmar_ratio'] = annualized_return / (metrics['max_drawdown_percent'] / 100)
                    else:
                    ...
                    # Calculate underwater periods
                    equity_df['underwater_group'] = (equity_df['is_underwater'] != equity_df['is_underwater'].shift()).cumsum()
                    underwater_periods = equity_df[equity_df['is_underwater']].groupby('underwater_group')
                    
                    if not underwater_periods.empty:
                        underwater_durations = underwater_periods.apply(lambda x: (x['datetime'].max() - x['datetime'].min()).total_seconds() / (60 * 60 * 24))  # days
    ...
    def _generate_plots(self, trades_df: pd.DataFrame, equity_df: pd.DataFrame, metrics: Dict):
        """
    ...
    def _plot_equity_curve(self, equity_df: pd.DataFrame, timestamp: str):
        ...
        try:
            ...
            if 'position' in equity_df.columns:
                # Long entries
                long_entries = equity_df[(equity_df['position'] == 'long') & (equity_df['position'].shift() != 'long')]
                if not long_entries.empty:
                    plt.scatter(long_entries['datetime'], long_entries['equity'], color='green', marker='^', s=50, label='Long Entry')
                    
                # Short entries
                short_entries = equity_df[(equity_df['position'] == 'short') & (equity_df['position'].shift() != 'short')]
                if not short_entries.empty:
                    plt.scatter(short_entries['datetime'], short_entries['equity'], color='red', marker='v', s=50, label='Short Entry')
                    
                # Position exits
                exits = equity_df[(equity_df['position'].isnull()) & (equity_df['position'].shift().notnull())]
                if not exits.empty:
                    plt.scatter(exits['datetime'], exits['equity'], color='black', marker='x', s=50, label='Exit')
    ...
    def _plot_drawdown(self, equity_df: pd.DataFrame, timestamp: str):
        ...
        try:
            ...
            # Plot drawdown
            plt.fill_between(equity_df['datetime'], equity_df['drawdown'], 0, color='red', alpha=0.3)
            plt.plot(equity_df['datetime'], equity_df['drawdown'], color='red', label='Drawdown')
    ...
    def _plot_monthly_returns(self, monthly_returns: Dict, timestamp: str):
        ...
        try:
            ...
            plt.xticks(range(len(monthly_series)), [str(idx) for idx in monthly_series.index], rotation=45)
    ...
    def _plot_equity_comparison(self, combined_equity: pd.DataFrame):
        ...
        try:
            # Convert datetime if it's a string
            if 'datetime' in combined_equity.columns and combined_equity['datetime'].dtype == 'object':
                combined_equity['datetime'] = pd.to_datetime(combined_equity['datetime'])
    ...
    def _generate_metrics_comparison(self, metrics_comparison: Dict) -> pd.DataFrame:
        ...
        try:
            ...
            for strategy, metrics in metrics_comparison.items():
                comparison_data[strategy] = {metric: metrics.get(metric, None) for metric in key_metrics}
                
            comparison_df = pd.DataFrame.from_dict(comparison_data)
            
            # Save comparison table
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            comparison_df.to_csv(self.output_dir / f"metrics_comparison_{timestamp}.csv")
