...
class MovingAverageCrossover(BaseStrategy):
    ...
    def generate_signals(self, data):
        ...
        for i in range(1, len(data)):
            ...
            if (
                data[fast_ma_col].iloc[i - 1] <= data[slow_ma_col].iloc[i - 1]
                and data[fast_ma_col].iloc[i] > data[slow_ma_col].iloc[i]
            ):
        ...
        for i in range(0, len(data)):
            ...
            if data["crossover"].iloc[i] == 1:
                ...
                if self.use_filters:
                    ...
                    if (
                        pd.isna(data[rsi_col].iloc[i])
                        or data[rsi_col].iloc[i] < self.rsi_overbought
                    ):
            ...
            # Check for sell signal (death cross)
                ...
                if self.use_filters:
                    ...
                    if (
                        pd.isna(data[rsi_col].iloc[i])
                        or data[rsi_col].iloc[i] > self.rsi_oversold
                    ):
                        data.loc[data.index[i], "signal"] = -1
                else:
                    data.loc[data.index[i], "signal"] = -1
