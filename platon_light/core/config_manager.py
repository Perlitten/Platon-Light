...
class ConfigManager:
    ...
    def set_api_credentials(self, service: str, credentials: Dict[str, str], 
        ...
        if encrypt and not self.encryption_key:
            logger.warning("Encryption not initialized, storing credentials unencrypted")
            encrypt = False
    ...
    def get_api_credentials(self, service: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if ('api_credentials' not in self.config or 
                service not in self.config['api_credentials']):
            ...
            if service_config['encrypted']:
                if not self.encryption_key:
                    logger.error("Cannot decrypt credentials: encryption not initialized")
                    return None
