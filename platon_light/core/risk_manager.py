...
class RiskManager:
    ...
    def __init__(self, config: Dict):
        ...
        # Risk limits
        self.max_risk_per_trade = self.risk_config.get("max_risk_per_trade_percentage", 1.0)
        ...
        # Circuit breakers
        self.daily_drawdown_warning = self.risk_config.get("daily_drawdown_limits", {}).get("warning", 3.0)
        self.daily_drawdown_soft_stop = self.risk_config.get("daily_drawdown_limits", {}).get("soft_stop", 5.0)
        self.daily_drawdown_hard_stop = self.risk_config.get("daily_drawdown_limits", {}).get("hard_stop", 10.0)
        ...
        self.price_history = {}
        self.abnormal_market_detection = self.risk_config.get("abnormal_market_detection", True)
        
        # Liquidation safety
        self.liquidation_safety_margin = self.risk_config.get("liquidation_safety_margin_percentage", 20.0)
    ...
    def update_balance(self, balance: float):
        ...
        self.daily_pnl_percentage = (self.daily_pnl / self.starting_balance) * 100 if self.starting_balance > 0 else 0
        
        # Check for circuit breakers
        if self.daily_pnl_percentage <= -self.daily_drawdown_hard_stop:
            if not self.trading_suspended:
                self.logger.warning(f"HARD STOP: Daily drawdown ({self.daily_pnl_percentage:.2f}%) exceeded hard limit ({self.daily_drawdown_hard_stop}%)")
            ...
            if not self.trading_suspended:
                self.logger.warning(f"SOFT STOP: Daily drawdown ({self.daily_pnl_percentage:.2f}%) exceeded soft limit ({self.daily_drawdown_soft_stop}%)")
                
        elif self.daily_pnl_percentage <= -self.daily_drawdown_warning:
            self.logger.warning(f"WARNING: Daily drawdown ({self.daily_pnl_percentage:.2f}%) exceeded warning level ({self.daily_drawdown_warning}%)")
            
        self.last_balance_update = current_time
        
    def reset_daily_stats(self):
        """Reset daily statistics (called at the start of a new trading day)"""
        self.logger.info(f"Resetting daily stats. Previous day P&L: {self.daily_pnl:.2f} ({self.daily_pnl_percentage:.2f}%)")
    ...
    async def validate_trade(self, symbol: str, signal: Dict, current_positions: List[Dict]) -> bool:
        """
        ...
        if self.trading_suspended:
            self.logger.warning(f"Trade validation failed: Trading suspended due to {self.suspension_reason}")
            return False
            
        # Check max open positions
        if len(current_positions) >= self.max_open_positions:
            self.logger.warning(f"Trade validation failed: Maximum open positions ({self.max_open_positions}) reached")
            return False
            
        # Check if we already have a position for this symbol
        for position in current_positions:
            if position["symbol"] == symbol:
                self.logger.warning(f"Trade validation failed: Position already exists for {symbol}")
                return False
                
        # Check risk per trade
        risk_percentage = signal["stop_loss_pct"]
        if risk_percentage > self.max_risk_per_trade:
            self.logger.warning(f"Trade validation failed: Risk per trade ({risk_percentage:.2f}%) exceeds maximum ({self.max_risk_per_trade}%)")
            return False
            
        # Check for abnormal market conditions
        if self.abnormal_market_detection and self.is_market_abnormal(symbol):
            self.logger.warning(f"Trade validation failed: Abnormal market conditions detected for {symbol}")
            return False
            
        # Check correlation with existing positions
        if len(current_positions) > 0 and not self.check_correlation(symbol, current_positions):
            self.logger.warning(f"Trade validation failed: {symbol} is too correlated with existing positions")
            return False
            
        # Check liquidation risk for futures
        if self.config["general"]["market_type"] == "futures" and not self.check_liquidation_risk(symbol, signal):
            self.logger.warning(f"Trade validation failed: Liquidation risk too high for {symbol}")
            return False
    ...
    async def check_trading_allowed(self) -> bool:
        ...
        if self.daily_pnl_percentage <= -self.daily_drawdown_soft_stop:
            self.logger.debug(f"Trading restricted: Daily drawdown ({self.daily_pnl_percentage:.2f}%) exceeded soft limit ({self.daily_drawdown_soft_stop}%)")
            return False
    ...
    def check_correlation(self, symbol: str, current_positions: List[Dict]) -> bool:
        ...
        for position in current_positions:
            ...
            symbol_returns = np.diff(symbol_prices[-min_length:]) / symbol_prices[-min_length:-1]
            ...
            if abs(correlation) > self.correlation_limit:
                self.logger.debug(f"Correlation between {symbol} and {pos_symbol} is {correlation:.2f}, exceeding limit {self.correlation_limit}")
                return False
    ...
    def check_liquidation_risk(self, symbol: str, signal: Dict) -> bool:
        ...
        liquidation_distance_pct = 100 / leverage * (1 - 1 / self.liquidation_safety_margin)
    ...
    def is_market_abnormal(self, symbol: str) -> bool:
        ...
        if current_volatility > avg_volatility * 3:
            self.logger.warning(f"Abnormal volatility detected for {symbol}: {current_volatility:.2f}% (avg: {avg_volatility:.2f}%)")
            return True
        ...
        if len(prices) >= 5:
            ...
            if price_change > 5:
                self.logger.warning(f"Abnormal price movement detected for {symbol}: {price_change:.2f}% in 5 minutes")
                return True
