...
from platon_light.utils.indicators import (
    calculate_rsi,
    calculate_macd,
...
class ScalpingStrategy:
    ...
    async def generate_signal(self, symbol: str, market_data: Dict) -> Optional[Dict]:
        ...
        for timeframe in self.timeframes:
            ...
            volume_percentile = np.percentile(
                volume[-50:], self.entry_config["min_volume_percentile"]
            )
            ...
            if order_book and "bids" in order_book and "asks" in order_book:
                ...
                strong_imbalance = (
                    imbalance_ratio > self.entry_config["min_order_book_imbalance"]
                )
            else:
                imbalance_ratio = 1.0
            ...
            if (
                rsi[-1] < self.entry_config["rsi"]["oversold"]
                and macd_hist[-1] > macd_hist[-2]
                and stoch_k[-1] < 30
                and stoch_k[-1] > stoch_d[-1]
            ):
            ...
            # Check for short signal
                ...
                rsi[-1] > self.entry_config["rsi"]["overbought"]
                and macd_hist[-1] < macd_hist[-2]
                and stoch_k[-1] > 70
                and stoch_k[-1] < stoch_d[-1]
            ):
        ...
        if self.exit_config["profit_target"]["type"] == "dynamic":
            profit_target_pct = (
                volatility * self.exit_config["profit_target"]["volatility_multiplier"]
            )
        else:
        ...
        if self.exit_config["stop_loss"]["type"] == "fixed":
            ...
            stop_loss_pct = min(
                ...
                self.exit_config["stop_loss"][
                    "initial_percentage"
                ],  # But not more than configured
            )
        ...
        signal = {
            ...
            "indicators": {
                tf: s["indicators"]
                for tf, s in zip([s["timeframe"] for s in signals_used], signals_used)
            },
        }

        # Log signal
        self.logger.info(
            f"Generated {direction.upper()} signal for {symbol} at {entry_price}"
        )
    ...
    def calculate_position_size(
        self, symbol: str, signal: Dict, balance: float
    ) -> float:
        """
        ...
        if method == "fixed":
            ...
            self.logger.warning(
                f"Unknown position sizing method: {method}, using default 5%"
            )
            return balance * 0.05
