...
class Position:
    ...
    def __init__(self, symbol: str, direction: str, entry_price: float, quantity: float,
                 target_price: float, stop_price: float, entry_time: int, order_id: str = None):
        """
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "status": self.status,
            "duration": (self.exit_time - self.entry_time) / 1000 if self.exit_time else None
        }
...
class PositionManager:
    ...
    def __init__(self, config: Dict, exchange: ExchangeClient, risk_manager: RiskManager):
        """
        ...
        self.max_trade_duration = self.exit_config.get("max_trade_duration_seconds", 300)
    ...
    async def execute_trade(self, symbol: str, signal: Dict) -> Optional[Dict]:
        ...
        # Get account balance
        base_currency = self.config["general"]["base_currency"]
        balance = await self.exchange.get_account_balance(base_currency)
        
        # Calculate position size
        from platon_light.core.strategy import ScalpingStrategy
        strategy = ScalpingStrategy(self.config, None)  # Temporary instance just for position sizing
        position_size = strategy.calculate_position_size(symbol, signal, balance)
        ...
        if self.market_type == "futures" and not self.is_dry_run:
            ...
            optimal_leverage = self.risk_manager.calculate_optimal_leverage(symbol, volatility)
        ...
        if "fills" in order and order["fills"]:
            ...
            entry_price = total_cost / total_qty if total_qty > 0 else float(order["price"])
        else:
    ...
    async def manage_position(self, position: Position, market_data: Dict) -> bool:
        ...
        take_profit_hit = (position.direction == "long" and current_price >= position.target_price) or \
                          (position.direction == "short" and current_price <= position.target_price)
                          
        # Check for stop loss
        stop_loss_hit = (position.direction == "long" and current_price <= position.stop_price) or \
                        (position.direction == "short" and current_price >= position.stop_price)
                        
        # Check for max trade duration
        max_duration_exceeded = (current_time - position.entry_time) / 1000 > self.max_trade_duration
        ...
        if self.trailing_stop and not position.trailing_activated:
            ...
            if position.direction == "long" and current_price >= (position.entry_price + half_target_pnl):
                position.trailing_activated = True
                self.logger.info(f"Trailing stop activated for {symbol} at {current_price}")
                
            elif position.direction == "short" and current_price <= (position.entry_price - half_target_pnl):
                position.trailing_activated = True
                self.logger.info(f"Trailing stop activated for {symbol} at {current_price}")
        ...
        if self.trailing_stop and position.trailing_activated:
            if position.direction == "long":
                ...
                if new_stop > position.stop_price:
                    ...
                    self.logger.info(f"Updated trailing stop for {symbol}: {old_stop:.8f} -> {new_stop:.8f}")
                    
                    # Update stop loss order if using futures
                    if self.market_type == "futures" and position.sl_order_id and not self.is_dry_run:
        ...
        if take_profit_hit or stop_loss_hit or max_duration_exceeded:
            reason = "take profit" if take_profit_hit else "stop loss" if stop_loss_hit else "max duration"
    ...
    async def close_position(self, symbol: str, reason: str = "manual") -> bool:
        ...
        if "fills" in order and order["fills"]:
            ...
            exit_price = total_cost / total_qty if total_qty > 0 else float(order["price"])
        else:
            # Use order price if no fills available
    ...
    def get_closed_positions(self, limit: int = None) -> List[Dict]:
        ...
        Returns:
            List of position dictionaries
        """
        positions = sorted(self.closed_positions, key=lambda p: p.exit_time or 0, reverse=True)
    ...
    def get_position_summary(self) -> Dict:
        ...
        total_pnl_percentage = sum(p["pnl_percentage"] for p in closed_positions) / len(closed_positions) if closed_positions else 0
        ...
        durations = [p["duration"] for p in closed_positions if p["duration"] is not None]
