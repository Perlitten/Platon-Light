...
class ConsoleVisualizer:
    ...
    def __init__(self, config: Dict):
        ...
        self.config = config
        self.last_update = 0
        self.update_interval = config.get("visualization", {}).get("update_interval", 2)  # seconds
    ...
    def update_display(self, trading_data: Dict, performance_data: Dict, active_positions: List[Dict]):
        """
    ...
    def _print_header(self):
        ...
        print(f"{Fore.CYAN}║{Style.RESET_ALL} {Fore.WHITE}{Style.BRIGHT}PLATON LIGHT - BINANCE SCALPING BOT{Style.RESET_ALL}{' ' * 44}{Fore.CYAN}║{Style.RESET_ALL}")
        print(f"{Fore.CYAN}║{Style.RESET_ALL} {current_time}{' ' * (78 - len(current_time))}{Fore.CYAN}║{Style.RESET_ALL}")
        print(f"{Fore.CYAN}╚{'═' * 78}╝{Style.RESET_ALL}")
        print()
    ...
    def _print_price_chart(self, trading_data: Dict):
        ...
        Args:
            trading_data: Dictionary of trading data
        """
        if not trading_data or "symbol" not in trading_data or "ohlcv" not in trading_data:
        ...
        # Extract OHLCV data
        timestamps = [datetime.fromtimestamp(ts / 1000).strftime("%H:%M:%S") for ts in [x[0] for x in ohlcv]]
        ...
        if current_price:
            ...
            print(f"Current Price: {price_color}{current_price:.8f}{Style.RESET_ALL} ({price_color}{price_change_pct:+.2f}%{Style.RESET_ALL})")
            
            if "rsi" in indicators:
                rsi = indicators["rsi"][-1] if indicators["rsi"] else None
                if rsi is not None:
                    rsi_color = Fore.GREEN if rsi < 30 else (Fore.RED if rsi > 70 else Fore.WHITE)
                    print(f"RSI: {rsi_color}{rsi:.2f}{Style.RESET_ALL}", end="  ")
                    
            if "macd" in indicators and "macd_signal" in indicators:
                macd = indicators["macd"][-1] if indicators["macd"] else None
                macd_signal = indicators["macd_signal"][-1] if indicators["macd_signal"] else None
    ...
    def _print_active_positions(self, active_positions: List[Dict]):
        ...
        # Print table
        headers = ["Symbol", "Side", "Quantity", "Entry Price", "Current Price", "PnL", "TP", "SL", "Open Time"]
    ...
    def _print_performance_metrics(self, performance_data: Dict):
        ...
        print(f"Avg Win: {Fore.GREEN}{avg_win:.8f}{Style.RESET_ALL}  |  "
    ...
    def _print_order_book(self, trading_data: Dict):
        ...
        if "current_price" in trading_data:
            ...
            print(f"{Fore.YELLOW}{'─' * 30} {current_price:.8f} {'─' * 30}{Style.RESET_ALL}")
            
        # Print bids
        for price, quantity in bids:
            bar_length = min(int(quantity * 100), 40)  # Scale the bar
            bar = "█" * bar_length
            print(f"{Fore.GREEN}BID: {price:.8f} | {quantity:.6f} {bar}{Style.RESET_ALL}")
    ...
    def print_trade_notification(self, trade: Dict):
        ...
        print(f"{side_color}{side.upper()}{Style.RESET_ALL} {symbol} | "
              f"Quantity: {quantity} | "
              f"Price: {price:.8f} | "
        ...
        for indicator, value in signals.items():
            if isinstance(value, bool):
                ...
                if indicator.lower() in ["rsi", "stoch"]:
                    color = Fore.GREEN if value < 30 else (Fore.RED if value > 70 else Fore.WHITE)
    ...
    def print_risk_assessment(self, risk_data: Dict):
        ...
        exposure_pct = (current_exposure / max_position_size) * 100 if max_position_size > 0 else 0
        
        daily_loss_limit = risk_data.get("daily_loss_limit", 0)
        current_daily_loss = risk_data.get("current_daily_loss", 0)
        loss_limit_pct = (current_daily_loss / daily_loss_limit) * 100 if daily_loss_limit != 0 else 0
        
        volatility = risk_data.get("volatility", 0)
        market_risk = risk_data.get("market_risk", "normal")
        
        # Format with colors
        exposure_color = Fore.GREEN if exposure_pct < 50 else (Fore.YELLOW if exposure_pct < 80 else Fore.RED)
        loss_color = Fore.GREEN if loss_limit_pct < 50 else (Fore.YELLOW if loss_limit_pct < 80 else Fore.RED)
        ...
        # Print metrics
        print(f"Exposure: {exposure_color}{exposure_pct:.2f}%{Style.RESET_ALL} of max ({current_exposure:.8f}/{max_position_size:.8f})")
        print(f"Daily Loss: {loss_color}{loss_limit_pct:.2f}%{Style.RESET_ALL} of limit ({current_daily_loss:.8f}/{daily_loss_limit:.8f})")
        print(f"Volatility: {volatility:.4f}  |  Market Risk: {market_risk_color}{market_risk}{Style.RESET_ALL}")
