...
class TelegramBot:
    ...
    async def start(self):
        ...
        if not self.token:
            self.logger.warning("Telegram bot token not provided, skipping initialization")
            return
    ...
    async def send_message(self, message: str, chat_id: Optional[int] = None):
        ...
        try:
            if chat_id:
                await self.app.bot.send_message(chat_id=chat_id, text=message, parse_mode=telegram.constants.ParseMode.MARKDOWN)
            else:
                for cid in self.chat_ids:
                    try:
                        await self.app.bot.send_message(chat_id=cid, text=message, parse_mode=telegram.constants.ParseMode.MARKDOWN)
                    except Exception as e:
    ...
    async def _handle_positions(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.callbacks["get_positions"]:
            ...
            for pos in positions:
                ...
                keyboard = [
                    [InlineKeyboardButton(f"Close {pos.get('symbol', '')}", callback_data=f"close_{pos.get('id', '')}")],
                ]
    ...
    async def _handle_start_trading(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.callbacks["start_trading"]:
            ...
            if result.get("success", False):
                ...
                await update.message.reply_text(f"❌ Failed to start trading: {result.get('error', 'Unknown error')}")
        else:
    ...
    async def _handle_stop_trading(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.callbacks["stop_trading"]:
            ...
            if result.get("success", False):
                ...
                await update.message.reply_text(f"❌ Failed to stop trading: {result.get('error', 'Unknown error')}")
        else:
    ...
    async def _handle_config(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not args:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("Strategy Parameters", callback_data="config_strategy")],
            ]
            ...
            if len(args) < 2:
                await update.message.reply_text("Invalid format. Use: /config parameter value")
                return
            ...
            if self.callbacks["update_config"]:
                ...
                if result.get("success", False):
                    ...
                    await update.message.reply_text(f"❌ Failed to update configuration: {result.get('error', 'Unknown error')}")
            else:
                await update.message.reply_text("Update config callback not registered.")
        ...
        if callback_data.startswith("close_"):
            ...
            if self.callbacks["close_position"]:
                ...
                if result.get("success", False):
                    ...
                    await query.edit_message_text(f"❌ Failed to close position: {result.get('error', 'Unknown error')}")
            else:
            ...
            if self.callbacks["get_status"]:
                ...
                if section == "risk":
                    ...
                    message += f"*Max Drawdown:* {risk_config.get('max_drawdown', 'N/A')}\n"
                    ...
                    message += f"*RSI Period:* {strategy_config.get('rsi_period', 'N/A')}\n"
                    ...
                    message += f"*MACD Fast:* {strategy_config.get('macd_fast', 'N/A')}\n"
                    message += f"*MACD Slow:* {strategy_config.get('macd_slow', 'N/A')}\n"
                    message += f"*MACD Signal:* {strategy_config.get('macd_signal', 'N/A')}\n"
