...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ab_test_strategies.log"),
...
class ABTester:
    ...
    def run_test(self, symbol, timeframe, start_date, end_date, config_a, config_b, 
                name_a="Strategy A", name_b="Strategy B", save_results=True):
        """
        ...
        """
        self.logger.info(f"Running A/B test for {symbol} from {start_date} to {end_date}")
        ...
        for metric in metrics:
            ...
            self.logger.info(f"{metric}: {value_a:.2f} vs {value_b:.2f} ({difference_str}) {result_symbol}")
        ...
        equity_curve_path = self.compare_equity_curves(results_a, results_b, name_a, name_b)
        
        # Compare drawdowns
        drawdown_path = self.compare_drawdowns(results_a, results_b, name_a, name_b)
        
        # Compare trade distributions
        trade_dist_path = self.compare_trade_distributions(results_a, results_b, name_a, name_b)
        
        # Compare monthly returns
        monthly_returns_path = self.compare_monthly_returns(results_a, results_b, name_a, name_b)
        ...
        if significance_results['is_significant']:
            ...
            if results_b['metrics']['sharpe_ratio'] > results_a['metrics']['sharpe_ratio']:
                self.logger.info(f"Slight preference for {name_b} based on Sharpe ratio")
                winner = f"{name_b} (slight preference)"
            else:
                self.logger.info(f"Slight preference for {name_a} based on Sharpe ratio")
    ...
    def test_statistical_significance(self, results_a, results_b, alpha=0.05):
        ...
        self.logger.info(f"Effect size (Cohen's d): {effect_size:.4f} ({effect_interpretation})")
    ...
    def compare_equity_curves(self, results_a, results_b, name_a="Strategy A", name_b="Strategy B"):
        """
    ...
    def compare_drawdowns(self, results_a, results_b, name_a="Strategy A", name_b="Strategy B"):
        """
    ...
    def compare_trade_distributions(self, results_a, results_b, name_a="Strategy A", name_b="Strategy B"):
        """
        ...
        """
        # Extract trade data
        trades_a = pd.DataFrame(results_a['trades']) if 'trades' in results_a else pd.DataFrame()
        trades_b = pd.DataFrame(results_b['trades']) if 'trades' in results_b else pd.DataFrame()
    ...
    def compare_monthly_returns(self, results_a, results_b, name_a="Strategy A", name_b="Strategy B"):
        """
        ...
        monthly_df = pd.DataFrame({
            name_a: monthly_returns_a,
            name_b: monthly_returns_b
        })
    ...
    def run_multi_period_test(self, symbol, timeframe, periods, config_a, config_b, 
        ...
        wins = sum(1 for result in period_results.values() 
                  if name_b in result['winner'])
        win_rate = (wins / len(periods)) * 100
        
        self.logger.info(f"\nOverall Results:")
        self.logger.info(f"{name_b} win rate across {len(periods)} periods: {win_rate:.2f}%")
        ...
        overall_results = {
            'test_parameters': {
                'symbol': symbol,
                'timeframe': timeframe,
                'periods': [(start.isoformat(), end.isoformat()) for start, end in periods],
                'strategy_a_name': name_a,
                'strategy_b_name': name_b
            },
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"{self.output_dir}/multi_period_test_{name_a}_vs_{name_b}_{timestamp}.json"
    ...
    def run_multi_asset_test(self, symbols, timeframe, start_date, end_date, config_a, config_b,
                            name_a="Strategy A", name_b="Strategy B"):
        """
        ...
        self.logger.info(f"Testing {name_a} vs {name_b} from {start_date} to {end_date}")
        ...
        # Calculate overall win rate for strategy B
        wins = sum(1 for result in asset_results.values() 
                  if name_b in result['winner'])
        win_rate = (wins / len(symbols)) * 100
        
        self.logger.info(f"\nOverall Results:")
        self.logger.info(f"{name_b} win rate across {len(symbols)} assets: {win_rate:.2f}%")
        ...
        # Save overall results
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"{self.output_dir}/multi_asset_test_{name_a}_vs_{name_b}_{timestamp}.json"
...
def parse_args():
    ...
    parser.add_argument('--strategy-a', type=str, required=True,
                        help='Name of strategy A')
    parser.add_argument('--strategy-b', type=str, required=True,
                        help='Name of strategy B')
    parser.add_argument('--config-a', type=str, required=True,
                        help='Path to config file for strategy A')
    parser.add_argument('--config-b', type=str, required=True,
                        help='Path to config file for strategy B')
    parser.add_argument('--output-dir', type=str, default='ab_test_results',
    ...
    parser.add_argument('--multi-asset', action='store_true',
                        help='Run test across multiple assets')
    parser.add_argument('--symbols', type=str, nargs='+',
                        help='List of symbols for multi-asset test')
...
def main():
    ...
    if args.multi_period:
        ...
        symbols = args.symbols if args.symbols else ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'ADAUSDT', 'XRPUSDT']
