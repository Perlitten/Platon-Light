...
class MonteCarloSimulator:
    ...
    def run_simulation(
        ...
        # Extract trade data
        trades = (
            pd.DataFrame(backtest_results["trades"])
        ...
        if (
            trades is None
            or trades.empty
            or "profit_loss_percent" not in trades.columns
        ):
    ...
    def simulate_trade_outcomes(
        self, trades, num_simulations=1000, confidence_level=0.95
    ):
        """
        ...
        self.plot_final_equity_distribution(
            final_equities, confidence_level, "trade_based"
        )
        ...
        self.logger.info(
            f"{confidence_level*100}% Confidence Interval: [{lower_bound:.2f}%, {upper_bound:.2f}%]"
        )
        self.logger.info(f"Probability of Profit: {prob_profit*100:.2f}%")
        self.logger.info(f"Mean Maximum Drawdown: {mean_max_drawdown:.2f}%")
        self.logger.info(
            f"Worst Drawdown (across all simulations): {worst_drawdown:.2f}%"
        )

        return results

    def simulate_returns(
        self, equity_curve, num_simulations=1000, confidence_level=0.95
    ):
        """
        ...
        for i in range(num_simulations):
            ...
            sampled_returns = np.random.choice(
                daily_returns, size=num_days, replace=True
            )
        ...
        # Plot final equity distribution
        self.plot_final_equity_distribution(
            final_equities, confidence_level, "returns_based"
        )

        # Plot drawdown distribution
        self.plot_drawdown_distribution(
            max_drawdowns, confidence_level, "returns_based"
        )
    ...
    def plot_equity_curves(self, equity_curves, confidence_level, simulation_type):
        ...
        plt.title(
            f"{simulation_type.capitalize()} Monte Carlo Simulation - Equity Curves"
        )
    ...
    def plot_final_equity_distribution(
        self, final_equities, confidence_level, simulation_type
    ):
        """
        ...
        plt.axvline(
            mean_equity, color="blue", linestyle="-", label=f"Mean: {mean_equity:.2f}%"
        )
        plt.axvline(
            median_equity,
            color="purple",
        ...
        plt.title(
            f"{simulation_type.capitalize()} Monte Carlo Simulation - Final Equity Distribution"
        )
    ...
    def plot_drawdown_distribution(
        self, max_drawdowns, confidence_level, simulation_type
    ):
        """
        ...
        plt.title(
            f"{simulation_type.capitalize()} Monte Carlo Simulation - Maximum Drawdown Distribution"
        )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Monte Carlo Simulation for Backtesting"
    )
    ...
    parser.add_argument(
        "--num-simulations", type=int, default=1000, help="Number of simulations to run"
    )
    parser.add_argument(
        "--confidence-level",
        type=float,
